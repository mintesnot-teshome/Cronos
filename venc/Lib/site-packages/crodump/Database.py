...
class Database:
    ...
    def _read_dbinfo_record(self):
        """
        Retrieve CroStru record #1, stripping leading garbage some datasets prefix.
        """
        if not self.stru:
            return
        ...
        if not dbinfo:
            return dbinfo
        cleaned = self._strip_leading_garbage(dbinfo)
        if cleaned != dbinfo:
            print("INFO: CroStru record #1 has %d unexpected leading bytes, skipping them" % (len(dbinfo) - len(cleaned)))
            dbinfo = cleaned
        if dbinfo[:1] != b"\x03":
            idx = dbinfo.find(b"\x03")
            if idx > 0:
                dbinfo = dbinfo[idx:]
        return dbinfo

    def _strip_leading_garbage(self, data):
        """
        Some v6/v7 datasets start records with a countdown byte pattern before the real payload.
        Detect and strip it so existing parsers continue to work.
        """
        if not data or len(data) < 12:
            return data
        prefix = data[:8]
        if all(((prefix[i] - prefix[i+1]) & 0xFF) == 1 for i in range(len(prefix) - 1)):
            return data[12:]
        return data
    ...
    def decode_db_definition(self, data):
        ...
        while not rd.eof():
            try:
                ...
                break
            safekey = keyname.encode("unicode_escape", errors="backslashreplace").decode("ascii")
            if keyname in d:
                print("WARN: duplicate key: %s" % safekey)
            ...
            if index_or_length >> 31:
                datalen = index_or_length & 0x7FFFFFFF
                try:
                    d[keyname] = rd.readbytes(datalen)
                except EOFError:
                    print("WARN: unexpected end of db definition while reading %s" % safekey)
                    break
            else:
                if index_or_length <= 0:
                    print("WARN: invalid CroStru reference %d for key %s" % (index_or_length, safekey))
                    continue
                try:
                    refdata = self.stru.readrec(index_or_length)
                except Exception as e:
                    print("WARN: failed to read CroStru record %d: %s" % (index_or_length, e))
                    continue
                refdata = self._strip_leading_garbage(refdata)
                if refdata[:1] != b"\x04":
    ...
    def dump_db_table_defs(self, args):
        ...
        """
        dbinfo = self._read_dbinfo_record()
        if not dbinfo:
            print("missing CroStru dbinfo")
            return
        if dbinfo[:1] != b"\x03":
        ...
        for k, v in dbdef.items():
            if k.startswith("Base") and k[4:].isnumeric():
                if files and k[4:] == "000":
                    try:
                        yield TableDefinition(self._strip_leading_garbage(v))
                    except Exception as e:
                        print("WARN: failed to decode table %s: %s" % (k, e))
                if not files and k[4:] != "000":
                    try:
                        yield TableDefinition(self._strip_leading_garbage(v), dbdef.get("BaseImage" + k[4:], b''))
                    except Exception as e:
                        print("WARN: failed to decode table %s: %s" % (k, e))
